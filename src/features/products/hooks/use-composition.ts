...
export function useComposition(productSku: string) {
  ...
  const loadCompositionItems = useCallback(async () => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to load composition items';
  ...
  const updateCompositionItem = useCallback(async (id: string, data: UpdateCompositionItemData) => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to update composition item';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setLoading(false);
