...
import { showErrorToast, showSuccessToast, OptimisticUpdate } from "@/lib/utils/error-handling";
...
export function useOptimisticList<TItem, TId = string>(
  ...
  const updateOptimistic = useCallback((id: TId, updates: Partial<TItem>) => {
    const currentItems = optimisticUpdateRef.current.getValue();
  ...
  const removeOptimistic = useCallback((id: TId) => {
    const currentItems = optimisticUpdateRef.current.getValue();
  ...
  const commit = useCallback((newItems?: TItem[]) => {
    if (newItems) {
  ...
  const setList = useCallback((newItems: TItem[]) => {
    optimisticUpdateRef.current = new OptimisticUpdate(newItems);
    triggerUpdate();
  }, [triggerUpdate]);
