...
export class ProductService {
  ...
  static validateProductData(
    ...
    if ('sku' in data && data.sku) {
      const duplicate = existingProducts.find(p => 
        p.sku === data.sku && p.sku !== excludeSku
      );
      if (duplicate) {
  ...
  /**
   * Validate product state transitions
   */
  static validateStateTransition(
    ...
    if (currentFlags.isComposite && !newFlags.isComposite) {
      const hasCompositionData = compositionItems.length > 0 || variations.length > 0;
      if (hasCompositionData) {
        warnings.push('Disabling composite will permanently delete all composition data');
      }
    }
    ...
    if (currentFlags.hasVariation && !newFlags.hasVariation) {
      if (variations.length > 0) {
        warnings.push('Disabling variations will permanently delete all variation data');
      }
    }

    // Validate enabling variations on composite
    if (!currentFlags.hasVariation && newFlags.hasVariation && newFlags.isComposite) {
      if (compositionItems.length > 0) {
        warnings.push('Existing composition will be migrated to the first variation');
      }
    }
