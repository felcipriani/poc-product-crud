...
describe("Business Rules - Referential Integrity", () => {
  describe("Variation Type Deletion Rules", () => {
    it("should define the rule: cannot delete variation type with existing variations", () => {
      const rule = {
        ...
        description:
          "A variation type cannot be deleted if it has variations associated with it",
    ...
    it("should define proper deletion order", () => {
      ...
      expect(deletionOrder[0]).toContain(
        "Remove variation from all product variations"
      );
  ...
  describe("Variation Deletion Rules", () => {
    it("should define the rule: cannot delete variation used in products", () => {
      const rule = {
        ...
        description:
          "A variation cannot be deleted if it is being used in product variations",
  ...
  describe("Unique Name Rules", () => {
    it("should define variation type name uniqueness rule", () => {
      ...
      expect(errorMsg).toBe(
        "A variation type with name 'Color' already exists"
      );
    });
    ...
    it("should define variation name uniqueness rule within type", () => {
      const rule = {
        ...
        description:
          "Variation names must be unique within their variation type (case-insensitive)",
        errorMessage: (name: string) =>
          `A variation with name '${name}' already exists in this variation type`,
        validation: (
          name: string,
          variationTypeId: string,
          existingVariations: Array<{ name: string; variationTypeId: string }>
        ) => {
          const normalizedName = name.toLowerCase().trim();
      ...
      expect(rule.validation("Green", "color-type", existingVariations)).toBe(
        true
      ); // New name in same type
      expect(rule.validation("Red", "size-type", existingVariations)).toBe(
        true
      ); // Same name in different type
      expect(rule.validation("red", "color-type", existingVariations)).toBe(
        false
      ); // Case-insensitive duplicate in same type
  ...
  describe("Data Integrity Rules", () => {
    it("should define cascade deletion prevention", () => {
      const rules = {
        ...
        description:
          "System should not automatically delete related entities. Users must explicitly delete in correct order.",
      };
  ...
  describe("User Experience Rules", () => {
    ...
    it("should define warning messages in UI", () => {
      const warnings = {
        variationType:
          "This variation type cannot be deleted if it has variations associated with it. Please delete all variations of this type first.",
        variation:
          "This variation cannot be deleted if it is being used in any product variations. Please remove it from all products first.",
      };
      ...
      expect(warnings.variation).toContain(
        "Please remove it from all products"
      );
    });
  });
});
