...
describe("ProductCompositionInterface", () => {
  ...
  beforeEach(async () => {
    ...
    const { useComposition } = await import(
      "@/features/products/hooks/use-composition"
    );
  ...
  describe("Non-composite products", () => {
    it("should show message for non-composite products", () => {
      ...
      expect(
        screen.getByText(/This product is not marked as composite/)
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Enable.*Composite Product.*in the Details tab/)
      ).toBeInTheDocument();
    });
  ...
  describe("Composite products", () => {
    ...
    it("should show empty state when no composition items exist", () => {
      ...
      expect(
        screen.getByText("No composition items configured yet")
      ).toBeInTheDocument();
    ...
    it("should open add modal when clicking Add Component button", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(
            "Select a product and specify the quantity to add to this composition"
          )
        ).toBeInTheDocument();
      });
    });
  ...
  describe("Composition items with data", () => {
    ...
    it("should allow updating quantities", async () => {
      ...
      await waitFor(() => {
        expect(mockUpdateCompositionItem).toHaveBeenCalledWith("comp-1", {
          quantity: 3,
        });
      });
    });
  ...
  describe("Error handling", () => {
    it("should display error messages", () => {
      ...
      expect(
        screen.getByText("Failed to load composition items")
      ).toBeInTheDocument();
    });
  });
