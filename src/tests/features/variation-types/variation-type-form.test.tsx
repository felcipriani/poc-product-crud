...
describe("VariationTypeForm", () => {
  ...
  it("validates required fields", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Variation type name is required")
      ).toBeInTheDocument();
    });
  ...
  it("validates name length", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Variation type name must be 50 characters or less")
      ).toBeInTheDocument();
    });
  });
  ...
  it("submits form with valid data", async () => {
    ...
    const modifiesDimensionsCheckbox =
      screen.getByLabelText(/modifies dimensions/i);
  ...
  it("shows information message when modification flags are selected", async () => {
    ...
    expect(
      screen.getByText(/when products use variations of this type/i)
    ).toBeInTheDocument();
  ...
  it("shows combined information when both flags are selected", async () => {
    ...
    expect(
      screen.getByText(/weight and dimensions.*will be ignored/i)
    ).toBeInTheDocument();
  });
