...
describe("CompositionService - Integration Tests", () => {
    ...
    it("should handle mixed composition with simple, composite, and variation products", async () => {
      ...
      mockProductRepository.findBySku.mockImplementation((sku: string) => {
        switch (sku) {
          ...
          case "CHAIR-VAR-001":
            return Promise.resolve(variableChair);
          case "LAMP-001":
            return Promise.resolve(simpleLamp);
          case "DESK-001":
            ...
            case "OFFICE-COMPLETE-001":
              return Promise.resolve([
                ...
                },
                {
                  id: "2",
                  parentSku: "OFFICE-COMPLETE-001",
                  childSku: "CHAIR-VAR-001#chair-var-red",
                  quantity: 2,
                },
                {
                  id: "3",
      ...
      const tree = await compositionService.getCompositionTree(
        "OFFICE-COMPLETE-001"
      );
      ...
      const chairChild = tree.children.find(
        (child) => child.sku === "CHAIR-VAR-001#chair-var-red"
      );
  ...
  describe("Composition complexity validation", () => {
    it("should analyze composition complexity and provide warnings", async () => {
      ...
      const analysis =
        await compositionService.validateCompositionComplexity("COMPLEX-001");
    ...
    it("should warn about excessive complexity", async () => {
      ...
      const analysis =
        await compositionService.validateCompositionComplexity("DEEP-001");
      ...
      expect(analysis.warnings).toEqual(
        expect.arrayContaining([
          expect.stringContaining(
            "Composition depth of 8 may impact performance"
          ),
        ])
      );
    });
  ...
  describe("Referential integrity reporting", () => {
    it("should generate comprehensive referential integrity report", async () => {
      ...
      const report =
        await compositionService.getReferentialIntegrityReport(
          "REPORT-TEST-001"
        );
    ...
    it("should handle non-existent product in report", async () => {
      ...
      const report =
        await compositionService.getReferentialIntegrityReport("NON-EXISTENT");
  ...
  describe("Cascade deletion scenarios", () => {
    it("should safely cascade delete composition items", async () => {
      ...
      expect(mockCompositionItemRepository.deleteByChild).toHaveBeenCalledWith(
        "DELETE-ME-001"
      );
      expect(mockCompositionItemRepository.deleteByChild).toHaveBeenCalledWith(
        "DELETE-ME-001#var-1"
