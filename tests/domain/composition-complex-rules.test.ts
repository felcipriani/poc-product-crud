...
describe('CompositionService - Complex Rules and Validation', () => {
  ...
  describe('Requirement 6.2: Individual variation combinations can be used in compositions', () => {
    ...
    it('should reject invalid variation SKUs', async () => {
      ...
      await expect(
        compositionService.validateChildProductEligibility('CHAIR-001#invalid-var')
    ...
    it('should reject variation SKU with mismatched product', async () => {
      ...
      await expect(
        compositionService.validateChildProductEligibility('CHAIR-001#var-123')
      ).rejects.toThrow('Variation \'var-123\' does not belong to product \'CHAIR-001\'');
    });
  });
  ...
  describe('Requirement 10.4: Referential integrity validation', () => {
    it('should detect circular dependencies', async () => {
      ...
      await expect(
        compositionService.validateReferentialIntegrity('DESK-SET-001', 'DRAWER-UNIT-001')
    ...
    it('should prevent duplicate children in same composition', async () => {
      ...
      mockCompositionItemRepository.findByParent.mockResolvedValue([
        { id: '1', parentSku: 'DESK-SET-001', childSku: 'CUSHION-001', quantity: 2 }
      ]);

      // Test: Should prevent duplicate
      await expect(
        compositionService.validateReferentialIntegrity('DESK-SET-001', 'CUSHION-001')
    ...
    it('should validate parent product is composite', async () => {
      ...
      await expect(
        compositionService.validateReferentialIntegrity('SIMPLE-001', 'CHILD-001')
  ...
  describe('Nested composition support', () => {
    it('should build composition tree with nested composites', async () => {
      ...
      mockProductRepository.findBySku
        .mockImplementation((sku: string) => {
          switch (sku) {
            case 'OFFICE-SET-001': return Promise.resolve(officeSet);
            case 'DESK-SET-001': return Promise.resolve(deskSet);
            case 'CHAIR-001': return Promise.resolve(chair);
      ...
      mockCompositionItemRepository.findByParent
        .mockImplementation((parentSku: string) => {
          switch (parentSku) {
            case 'OFFICE-SET-001':
              return Promise.resolve([
                { id: '1', parentSku: 'OFFICE-SET-001', childSku: 'DESK-SET-001', quantity: 1 },
                { id: '2', parentSku: 'OFFICE-SET-001', childSku: 'CHAIR-001', quantity: 2 },
              ]);
            case 'DESK-SET-001':
              return Promise.resolve([
                { id: '3', parentSku: 'DESK-SET-001', childSku: 'DESK-001', quantity: 1 },
              ]);
            default:
              return Promise.resolve([]);
          }
      ...
      const tree = await compositionService.getCompositionTree('OFFICE-SET-001');
    ...
    it('should prevent excessive nesting depth', async () => {
      ...
      mockCompositionItemRepository.findByParent.mockImplementation((parentSku: string) => {
        ...
        if (level < 15) {
          return Promise.resolve([
            { id: `${level}`, parentSku, childSku: `LEVEL-${level + 1}`, quantity: 1 }
          ]);
        }
        return Promise.resolve([]);
      });
  ...
  describe('Product deletion impact validation', () => {
    it('should identify products that cannot be deleted due to composition usage', async () => {
      ...
      mockCompositionItemRepository.findByChild.mockResolvedValue([
        { id: '1', parentSku: 'OFFICE-SET-001', childSku: 'CHAIR-001', quantity: 2 },
        { id: '2', parentSku: 'DESK-SET-001', childSku: 'CHAIR-001', quantity: 1 },
      ]);
      ...
      const impact = await compositionService.validateProductDeletionImpact('CHAIR-001');
      ...
      expect(impact.affectedCompositions).toEqual(['OFFICE-SET-001', 'DESK-SET-001']);
    });
    ...
    it('should allow deletion of products not used in compositions', async () => {
      ...
      const impact = await compositionService.validateProductDeletionImpact('UNUSED-001');
  ...
  describe('Variation SKU parsing', () => {
    ...
    it('should parse variation SKU with VAR format', async () => {
      ...
      await expect(
        compositionService.validateChildProductEligibility('CHAIR-001-VAR-abc123')
      ).resolves.not.toThrow();
    });
  });
});