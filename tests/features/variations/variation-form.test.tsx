...
describe('VariationForm', () => {
  ...
  it('pre-selects variation type when provided', () => {
    render(
      <VariationForm
        {...defaultProps}
        selectedVariationTypeId="2"
      />
    );

    const select = screen.getByLabelText(/variation type/i) as HTMLSelectElement;
  ...
  it('disables variation type selection when editing', () => {
    ...
    expect(screen.getByText(/variation type cannot be changed when editing/i)).toBeInTheDocument();
  });
  ...
  it('validates name length', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Variation name must be 50 characters or less')).toBeInTheDocument();
    });
  });
  ...
  it('shows variation type information when type is selected', async () => {
    ...
    expect(screen.getByText((content, element) => {
      return element?.tagName === 'P' && element?.textContent?.trim() === 'Color variations are aesthetic only';
    })).toBeInTheDocument();
  });
  ...
  it('shows weight modification info for weight-modifying types', async () => {
    ...
    expect(screen.getByText((content, element) => {
      return element?.tagName === 'P' && element?.textContent?.trim() === 'Material variations modify weight';
    })).toBeInTheDocument();
  });
  ...
  it('shows dimension modification info for dimension-modifying types', async () => {
    ...
    expect(screen.getByText((content, element) => {
      return element?.tagName === 'P' && element?.textContent?.trim() === 'Size variations modify both weight and dimensions';
    })).toBeInTheDocument();
  });
