...
import { ValidationError, BusinessRuleError, StorageError } from '@/lib/utils/error-handling';
import { CreateProductData, UpdateProductData } from '@/lib/domain/entities/product';
...
describe('Data Persistence Integration', () => {
  ...
  describe('Product Update with Error Handling', () => {
    it('should update a product successfully', async () => {
      ...
      // Mock existing product
      localStorageMock.getItem.mockReturnValue(JSON.stringify([existingProduct]));
    ...
    it('should handle product not found during update', async () => {
      ...
      await expect(productService.updateProduct('NONEXISTENT', updateData))
        .rejects
        .toThrow(ValidationError);
    });
    ...
    it('should validate business rules during update', async () => {
      ...
      await expect(productService.updateProduct('TEST-001', updateData))
        .rejects
        .toThrow(BusinessRuleError);
    });
  });
  ...
  describe('Product Deletion with Error Handling', () => {
    it('should delete a product successfully', async () => {
      ...
      await expect(productService.deleteProduct('TEST-001')).resolves.not.toThrow();
    });

    it('should handle product not found during deletion', async () => {
      localStorageMock.getItem.mockReturnValue('[]');

      await expect(productService.deleteProduct('NONEXISTENT'))
        .rejects
        .toThrow(ValidationError);
    });
    ...
    it('should prevent deletion of products used in compositions', async () => {
      ...
      await expect(productService.deleteProduct('TEST-001'))
        .rejects
        .toThrow(BusinessRuleError);
    });
  });
