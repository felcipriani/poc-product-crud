...
describe("Repository Validation Logic", () => {
  describe("Error Message Validation", () => {
    it("should generate correct error message for variation type deletion", () => {
      ...
      expect(error1).toBe("Cannot delete variation type 'Color' because it has 1 variation(s) associated with it. Please delete all variations first.");
      expect(error3).toBe("Cannot delete variation type 'Size' because it has 3 variation(s) associated with it. Please delete all variations first.");
    ...
    it("should generate correct error message for variation deletion", () => {
      ...
      expect(error1).toBe("Cannot delete variation 'Red' because it is being used in 1 product variation(s). Please remove it from all products first.");
      expect(error2).toBe("Cannot delete variation 'Blue' because it is being used in 2 product variation(s). Please remove it from all products first.");
    ...
    it("should generate correct error message for duplicate names", () => {
      ...
      expect(vError).toBe("A variation with name 'Red' already exists in this variation type");
  ...
  describe("Name Validation Logic", () => {
    it("should validate case-insensitive name comparison", () => {
      const normalizeForComparison = (name: string) => name.toLowerCase().trim();
  ...
  describe("Referential Integrity Logic", () => {
    it("should validate deletion constraints", () => {
      const canDeleteVariationType = (variationCount: number) => variationCount === 0;
  ...
  describe("Business Rule Validation", () => {
    it("should validate creation rules", () => {
      ...
      expect(validateCreation("", existing)).toEqual({ valid: false, error: "Name is required" });
      expect(validateCreation("color", existing)).toEqual({ valid: false, error: "Name 'color' already exists" });
      expect(validateCreation("a".repeat(51), existing)).toEqual({ valid: false, error: "Name too long" });
    });

    it("should validate update rules", () => {
      const validateUpdate = (id: string, name: string, existingItems: Array<{id: string, name: string}>) => {
        if (!name || name.trim().length === 0) {
          return { valid: false, error: "Name is required" };
        }
      ...
      expect(validateUpdate("1", "Material", existing)).toEqual({ valid: true }); // New name
      expect(validateUpdate("1", "size", existing)).toEqual({ valid: false, error: "Name 'size' already exists" }); // Duplicate
    });
  });
});