...
describe('CompositionService - Composite Variations (Requirement 7)', () => {
  ...
  describe('Requirement 7.1: Composition-based variation interface', () => {
    it('should identify products that need composition-based variation interface', async () => {
      ...
      mockProductRepository.findBySku.mockResolvedValue(compositeVariationProduct);
      mockVariationItemRepository.findByProduct.mockResolvedValue([]);

      const result = await compositionService.getCompositeVariationsWithComposition('DINING-SET-001');

      expect(result).toEqual([]);
      expect(mockProductRepository.findBySku).toHaveBeenCalledWith('DINING-SET-001');
    });
    ...
    it('should return empty array for non-composite or non-variation products', async () => {
      ...
      const result = await compositionService.getCompositeVariationsWithComposition('SIMPLE-001');
  ...
  describe('Requirement 7.2: Selection of specific child variations per combination', () => {
    it('should create composition items for specific variation combinations', async () => {
      ...
      vi.spyOn(compositionService, 'validateChildProductEligibility').mockResolvedValue();
      ...
      vi.spyOn(compositionService, 'hasCircularDependency').mockResolvedValue(false);
  ...
  describe('Requirement 7.3: Weight calculation from selected child variations', () => {
    it('should calculate weight from composition items when no weight override', async () => {
      ...
      vi.spyOn(compositionService, 'calculateCompositeWeight').mockResolvedValue(36); // 4*5 + 1*16
      ...
      expect(compositionService.calculateCompositeWeight).toHaveBeenCalledWith('DINING-SET-001#var-modern');
    });
    ...
    it('should use weight override when parent variation type modifies weight', async () => {
      ...
      mockProductRepository.findBySku.mockResolvedValue(compositeVariationProduct);
      mockVariationItemRepository.findById.mockResolvedValue(variationWithWeightOverride);

      // Mock variation types to include weight-modifying type
      vi.spyOn(compositionService as any, 'getVariationTypesForProduct').mockResolvedValue([
        { id: 'packaging-type', modifiesWeight: true },
      ]);
  ...
  describe('Requirement 7.4: Combination uniqueness validation', () => {
    it('should prevent duplicate variation combinations', async () => {
      ...
      mockVariationItemRepository.findById.mockResolvedValue(duplicateVariation);
      mockVariationItemRepository.findByProduct.mockResolvedValue([existingVariation]);

      await expect(
        compositionService.validateCompositeVariationUniqueness('DINING-SET-001', 'var-duplicate')
      ).rejects.toThrow('variation combination with the same selections already exists');
    });
    ...
    it('should allow unique variation combinations', async () => {
      ...
      await expect(
        compositionService.validateCompositeVariationUniqueness('DINING-SET-001', 'var-unique')
    ...
    it('should exclude specified variation from uniqueness check', async () => {
      ...
      await expect(
        compositionService.validateCompositeVariationUniqueness('DINING-SET-001', 'var-1', 'var-1')
      ).resolves.not.toThrow();
    });
  });
  ...
  describe('Requirement 7.5: Variation selection for child products with variations', () => {
    it('should validate completeness of composite variation composition', async () => {
      ...
      const result = await compositionService.validateCompositeVariationCompleteness(
        'DINING-SET-001',
        'var-modern'
      );
    ...
    it('should identify missing composition items', async () => {
      ...
      expect(result.missingItems).toContain('At least one composition item is required');
    });
  ...
  describe('Composition management operations', () => {
    it('should get composition items for specific variation', async () => {
      ...
      expect(mockCompositionItemRepository.findByParent).toHaveBeenCalledWith('DINING-SET-001#var-modern');
    });
    ...
    it('should update composition for variation by replacing all items', async () => {
      ...
      vi.spyOn(compositionService, 'createCompositeVariationComposition').mockResolvedValue();
      ...
      expect(mockCompositionItemRepository.deleteByParent).toHaveBeenCalledWith(variationSku);
      expect(compositionService.createCompositeVariationComposition).toHaveBeenCalledWith(
        'DINING-SET-001',
        'var-modern',
        [
          { childSku: 'CHAIR-001#chair-red', quantity: 2 },
    ...
    it('should get all composite variations with their details', async () => {
      ...
      vi.spyOn(compositionService, 'validateCompositeVariationCompleteness')
        .mockResolvedValueOnce({ isComplete: false, missingItems: ['No items'], invalidItems: [] })
        .mockResolvedValueOnce({ isComplete: false, missingItems: ['No items'], invalidItems: [] });
  ...
  describe('Error handling', () => {
    it('should handle non-existent variation in weight calculation', async () => {
      ...
      await expect(
        compositionService.calculateCompositeVariationWeight('DINING-SET-001', 'non-existent')
    ...
    it('should handle variation belonging to different product', async () => {
      ...
      await expect(
        compositionService.calculateCompositeVariationWeight('DINING-SET-001', 'var-1')
      ).rejects.toThrow('Variation \'var-1\' not found for product \'DINING-SET-001\'');
    });
  });
});